...
<ul>
<li> 数据需要包括<b>一个因变量（Y） </b> 和<b> 至少一个自变量（X）</b>
<li> 数据的行数必须多于列数。
<li> 不要在同一列中混用字符和数字
...
<ul>
<li> 建立简单或多元线性回归模型 
<li> 得出回归估计，包括（1）用t检验、p值和95% CI进行系数估计，（2）R<sup>2</sup> 和经调整的R<sup>2</sup>，（3）F检验进行回归的总体显著性
<li> 获得更多信息：（1）预测因变量和残差，（2）模型的ANOVA表，（3）基于AIC的变量选择，（4）基于残差和预测因变量的诊断图
<li> 上传新数据，得到预测
<li> 对新数据包含新因变量的评价
</ul>
