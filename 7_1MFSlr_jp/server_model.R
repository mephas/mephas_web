...
shinyWidgets::pickerInput(
'conf',
tags$b('4 （オプション） 2つのカテゴリ変数間に交互作用項を追加する'),
choices = type.fac4(),
multiple = TRUE,
options = shinyWidgets::pickerOptions(
      maxOptions=2,
...
## LM formula
formula = reactive({
validate(need(input$x, "いくつかの独立変数を選択してください"))
...
selectInput(
'vx1',
tags$b('1. 1つの独立変数（X1、実数値の数値型）を選択する'),
...
selectInput(
'vx2',
tags$b('2. 1つの独立変数（X2、実数値の数値型）を選択する'),
...
selectInput(
'vgroup',
tags$b('3. グループ変数、カテゴリタイプを1つ選択する'),
selected = "NULL",
