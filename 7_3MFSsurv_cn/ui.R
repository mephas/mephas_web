...
<li> 准备<b>生存对象</b>替代模型中的<b>因变量</b></li>
</ul>

<h4><b> 2. 关于数据（训练集）</b></h4>

<ul>
<li> 数据需要包括<b>一个生存时间变量和一个1/0审查变量</b> 以及 <b> 至少一个自变量（X）。</b></li>
<li> 数据的行数必须多于列数。</li>
<li> 不要在同一列中混用字符和数字。</li>
<li> 用于构建模型的数据称为<b>训练集</b>。</li>
</ul>

<h4><i>示例1： 右删失糖尿病数据</i></h4>
<i>假设在一项研究中从激光凝固治疗糖尿病视网膜病变的试验中得到一些观察值。每个患者有一只眼随机接受激光治疗，另一只眼不接受治疗。对于每只眼，侧重事件是从开始治疗到连续两次访视视力下降到5/200以下的时间。因此，存在大约6个月的固有滞后时间（每3个月访视一次）。因此，本数据集中的生存时间为实际致盲时间（以月为单位）减去发生事件的最小可能时间（6.5个月）。
删失状态0 = 已删失；1 = 视力下降。治疗：0 = 无治疗，1 = 激光。年龄是诊断时的年龄。</i>
</i>


<h4><i>示例2： 左截断右截尾Nki70数据</i></h4>
<i>假设想探讨100例淋巴结阳性乳腺癌患者的无转移生存率。但有些患者比其他患者入组晚。数据包含5个临床危险因素：（1）Diam：肿瘤的直径；（2）N：受影响的淋巴结数量；（3）ER：雌激素受体状态；（4）分级：肿瘤的分级；(5)年龄：患者诊断时的年龄（岁）；
以及在早期研究中发现对无转移生存期有预后意义的70个基因的基因表达测量。时间变量为无转移随访时间（月）。删失指标变量：1 = 转移或死亡；0 = 删失。</i>
<br><br>
<p>探讨生存期与自变量之间的关系。<p>
</i>
...
<h4><b> 1. 功能</b></h4>
<ul>
<li> 获得Kaplan-Meier存活率估计</li>
<li> 通过各个变量获得Kaplan-Meier生存曲线、累积事件分布曲线和累积危险曲线</li>
<li> 进行对数秩检验，比较两组的生存曲线</li>
<li> 进行配对对数秩检验，比较三组以上的生存曲线</li>
</ul>
...
<ul>
<li> 建立Cox回归模型</li>
<li> 获得模型的估计，例如（1）系数的估计，（2）训练数据的预测，（3）残差，（4）调整的生存曲线，（5）比例危险试验，（6）诊断图</li>
<li> 上传新数据，得到预测</li>
<li> 对新数据包含的新因变量进行评价</li>
<li> 获得Brier评分和时间依赖性AUC</li>
</ul>
...
<ul>
<li> 请在“数据”选项卡中准备训练数据</li>
<li> 请在“数据”选项卡中准备生存对象，生存（时间、事件）</li>
