...
<ul>
<li>rho = 0：对数秩或Mantel-Haenszel检验
<li>rho = 1：Gehan-Wilcoxon检验的Peto&Peto修正
<li>p < 0.05表示存活率曲线有显著性差异
<li>p >= 0.05表示存活率曲线无显著性差异
...
HTML(
"<b> 説明 </b>
<p>在这里，用每次死亡对S(t)<sup>rho</sup>进行加权。 
Harrington和Fleming（1982）开发了G-rho系列方法。
其中S(t)是Kaplan-Meier生存估计。</p>
<ul> 
<li><b>rho = 0:</b> 对数秩或Mantel-Haenszel检验
<li><b>rho = 1:</b> Gehan-Wilcoxon检验的Peto&Peto修正
...
<li> P值 < 0.05表示生存率曲线有显著性差异
<li> P值 >= 0.05表示生存率曲线无显著性差异
</ul>"
),
p(tags$b("成对对数秩检验P值表")),
