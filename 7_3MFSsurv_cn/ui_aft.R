...
p(tags$b("1.  在“数据”选项卡中选中生存对象，Surv(time, event)或者Surv(time, event)")), 
...
uiOutput('fx.c'),
tags$i("在糖尿病数据的示例中，“eye”可用作分层随机效应， “id”可以用作聚类的随机效应变量。" )
)
),
hr(),
...
p(br()),
actionButton("B1", (tags$b("结果表示 >>")),class="btn btn-primary",icon=icon("bar-chart-o")),
p(br()),
p(br()),
hr()
),
...
<b> 说明  </b>
<ul>
<li> 对于每个变量，给出了系数的估计值、单个变量的显著性统计量和P值
<li> 标记为“z”的列给出Wald统计量。与各回归系数与其标准差的比值相对应（z = coef/ se (coef)）。Wald统计量用来检验系数的估计值在统计上是否与0有显著差异
<li> 系数与风险（hazard）有关；正系数表示预后较差，负系数表示与之相关的变量的保护作用
<li> exp（系数的估计值） = 风险比（Hazrd Ratio, HR）。HR = 1：没有效果；HR<1：危险降低；HR>1：危险增加
<li> Scale和Log(scale)是AFT模型误差项中的估计参数
<li>　模型中给出了对数似然。当使用最大似然估生成对数似然时，对数似然估计（LL）越接近于零，模型拟合越好
...
<li> 模型拟合根据其AIC值秩和，AIC值最低的模型通常被认为是“最佳”模型
</ul>
</ul>"
),
...
<ul>
<li>鞅残差值在1附近表示“过早死亡”的个体
<li>较大的负值对应于“寿命过长”的个体
</ul>

<b>异常残差(Deviance residual)</b>是鞅残差的归一化变换。这些残差应大致对称地分布在零左右，标准偏差为1
<ul>
<li>与预期存活时间相比，正值对应“死得太快”的个体
<li>负值对应“活得太久”的个体
<li>很大或很小的值都是离群值，模型预测很差
</ul>

<b>Cox-Snell残差</b>用于检验生存模型的总体拟合优度
<ul>
<li> Cox-Snell残差等于每个观察的-读数（存活率）
<li> 如果模型能很好地拟合数据，Cox-Snell残差应位于45度对角线上
</ul>
...
uiOutput('var.mr2'),
plotly::plotlyOutput("mrplot", width = 700),

p(tags$b("2. 观察ID和异常残差图")),
plotly::plotlyOutput("deplot", width = 700),

p(tags$b("3. Cox-Snell残差图")),
plotly::plotlyOutput("csplot", width = 700)
