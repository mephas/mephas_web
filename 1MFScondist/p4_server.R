...
output$b.plot <- renderPlot({
  ...
  stat_function(fun = "dbeta", args = list(shape1 = input$b.shape, shape2=input$b.scale)) + ylab("Density") +
  scale_y_continuous(breaks = NULL) + theme_minimal() + ggtitle("") + #ylim(0, input$b.ylim) +
...
xlim(-0.1, input$b.xlim) + geom_vline(aes(xintercept=quantile(x, probs = input$b.pr, na.rm = FALSE)), color="red", size=0.5)
})
