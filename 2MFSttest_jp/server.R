...
output$makeplot <- renderPlot({
  x <- Z()
  plot1 <- ggplot(x, aes(sample = x[, 1])) + stat_qq() + ggtitle("正規Q-Qプロット") + xlab("") + theme_minimal()  ## add line,
  plot2 <- ggplot(x, aes(x = x[, 1])) + geom_histogram(colour = "black",fill = "grey",binwidth = input$bin,position = "identity") + xlab("") + ggtitle("ヒストグラム") + theme_minimal() + theme(legend.title =element_blank())
  plot3 <- ggplot(x, aes(x = x[, 1])) + geom_density() + ggtitle("密度プロット") + xlab("") + theme_minimal() + theme(legend.title =element_blank())
...
output$makeplot2 <- renderPlot({
  ...
  # normal qq plot
  plot1 <- ggplot(x, aes(sample = x[, 1])) + stat_qq(color = "brown1") + ggtitle(paste0("正規Q-Qプロット ", colnames(x[1]))) + theme_minimal()
  plot2 <- ggplot(x, aes(sample = x[, 2])) + stat_qq(color = "forestgreen") + ggtitle(paste0("正規Q-Qプロット ", colnames(x[2]))) + theme_minimal()
  ...
  plot3 <- ggplot(mx, aes(x = "value", colour = "variable", fill = "variable")) + 
    ...
    ggtitle("ヒストグラム") + xlab("") + theme_minimal() + theme(legend.title = element_blank())
  plot4 <- ggplot(mx, aes(x = "value", colour = "variable")) + geom_density() + 
    ggtitle("密度プロット") + xlab("") + theme_minimal() + theme(legend.title = element_blank())
...
output$makeplot.p <- renderPlot({
  x <- Z()
  plot1 <- ggplot(x, aes(sample = x[, 3])) + stat_qq() + ggtitle("平均の差の正規Q-Qプロット") + xlab("") + theme_minimal()  ## add line,
  plot2 <- ggplot(x, aes(x = x[, 3])) + geom_histogram(colour = "black",fill = "grey", binwidth = input$bin.p, position = "identity") + xlab("") + ggtitle("ヒストグラム") + theme_minimal() + theme(legend.title =element_blank())
  plot3 <- ggplot(x, aes(x = x[, 3])) + geom_density() + ggtitle("密度プロット") + xlab("") + theme_minimal() + theme(legend.title = element_blank())
