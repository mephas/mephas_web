dtametasa <- function(
  ...
  if (!inherits(opt, "try-error")) {
    ...
    if (p == 1) {
      ...
      try.inv.I.fun.m <- try(solve(hes[1:5, 1:5]), silent = show.warn.message)
      
    if(!inherits(try.inv.I.fun.m, "try-error")) inv.I.fun.m <- try.inv.I.fun.m else inv.I.fun.m <- matrix(NA, 5, 5)
    
  }  else {
      
      # inv.I.fun.m <- solve(hes)
      
      try.inv.I.fun.m <- try(solve(hes), silent = show.warn.message)
      
      if(!inherits(try.inv.I.fun.m, "try-error")) inv.I.fun.m <- try.inv.I.fun.m else inv.I.fun.m <- matrix(NA, 7, 7)
    ...
    if (p == 1) 
      ...
      a.p <- function(a) {
        sum(1/pnorm((a + b * u.ldor/se.ldor)/sq), na.rm = TRUE) - n/p
      }
