dtametasa_fc<-function (data, p, c1.square = 0.5, beta0 = 1, reitsma.par0 = NULL, 
                      beta.interval = c(0, 2), alpha.interval = c(-3, 3), ci.level = 0.95, 
                      correct.value = 0.5, correct.type = c("single", "all")[1], 
                      show.warn.message = FALSE, alpha.root.extendInt = "downX", 
                      eps = sqrt(.Machine$double.eps), sauc.type = c("sroc", "hsroc")[1], 
                      ...) 
{
  if (p <= 0 | p > 1) 
  ...
  if (!length(reitsma.par0) == 5) {
    start6 <- c(0, 0, 0.1, 0.1, -0.1, beta0)
    fit.m <- mixmeta::mixmeta(cbind(y1, y2), S = cbind(v1, 
                                                       rep(0, n), v2), method = "ml")
    if (!inherits(fit.m, "try-error")) {
      if (fit.m$converged) {
        p1 <- sqrt(fit.m$Psi[1])
        p2 <- sqrt(fit.m$Psi[4])
        p.r <- fit.m$Psi[3]/(p1 * p2)
        start6 <- c(fit.m$coefficients, p1, p2, p.r, 
                    beta0)
      }
    }
  }
  else start6 <- c(reitsma.par0, beta0)
  names(start6) <- c("mu1", "mu2", "tau1", "tau2", "rho", 
                     "beta")
