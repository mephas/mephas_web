...
headerPanel("单样本威尔科克森符号秩检验（Wilcoxon Signed-Rank Test）"),

conditionalPanel(
condition = "input.explain_on_off",
HTML(
    "
<p>数据不能假定为正态分布时，这种方法是单样本 t 检验的替代方法。这种方法基于观测值的排序，而不是基于实际值</p>

<h4><b> 1. 功能  </b></h4>
<ul>
<li> 测定数据来源的总体的中位数/位置在统计学上是否与指定的中位数有显著差异</li>
<li> 了解有关数据的基本描述性统计信息</li>
<li> 了解数据的描述性统计图，如箱形图、分布直方图及密度分布图</li>
</ul>

<h4><b> 2. 关于数据 </b></h4>

<ul>
<li> 数据仅包含1组值（或 1 个数字向量）</li>
<li> 数据对测量与指定中位数的距离有意义</li>
<li> 该组值是独立观察值</li>
<li> 没有假设数据的分布形状，表示数据可能不是正态分布的</li>
</ul>

<h4><i>示例</i></h4>
<i>假设从一组特定患者中收集了9名患者的抑郁评定量表(DRS) 测量结果。DRS评分>1表示“抑郁”。想知道患者的DRS是否显著大于1。
</i>
...
headerPanel("两个独立样本的威尔科克森秩和检验（曼-惠特尼 U 检验） Wilcoxon Rank-Sum Test (Mann–Whitney U test)"),
...
HTML(
    ...
    "
<p>数据不能假定为正态分布时，这种方法是双样本 t 检验的替代方法。</p>

<h4><b> 1. 功能  </b></h4>
<ul>
<li> 测定2组数据的两个总体中位数在统计学上是否存在显著差异</li>
<li> 测定2组数据的分布位置是否不同</li>
<li> 了解有关数据的基本描述性统计信息</li>
<li> 了解数据的描述性统计图，如箱形图、分布直方图及密度分布图</li>
</ul>
...
<ul>
<li> 数据仅包含2组值（或2个数字向量）</li>
<li> 数据对测量2组值之间的距离有意义</li>
<li> 该组值是独立观察值</li>
<li> 不假设数据的分布形状</li>
<li> 数据可能不是正态分布</li>
</ul>
...
<h4><i>示例</i></h4>
<i>假设从一组特定患者中收集了19名患者的抑郁评定量表(DRS) 测量结果。19人中有9名女性，10名男性。我们想知道不同性别患者的DRS是否有显著性差异；或者，年龄是否与DRS评分有关。
</i>
<h4>请按照以下<b>步骤</b>，<b>输出</b>实时分析结果。</h4>
...
<h4><b> 1. 功能  </b></h4>
<ul>
<li> 以测定配对数据的差异是否在统计学上与 0 显著不同</li>
<li> 了解有关数据的基本描述性统计信息</li>
<li> 了解数据的描述性统计图，如箱形图、分布直方图及密度分布图</li>
</ul>
...
<h4><i>示例</i></h4>
<i>我们想知道患者治疗前后的DRS是否有显著；或者，差异是否与0显著不同，这可以表明治疗是否有效。
</i>
<h4>请按照以下<b>步骤</b>，<b>输出</b>实时分析结果。</h4>
"
)
