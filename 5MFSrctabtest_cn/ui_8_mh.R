...
conditionalPanel(
    ...
    p(tags$b("K层2x2列联表")),
    p("前两行显示第一层的2 x 2列联表，然后显示第二层的2 x 2列联表。"),
    ...
    "<b> 说明 </b> 
    <ul> 
    <li> P值 < 0.05，则在个人吸烟得到控制的情况下，二手烟与患癌症的风险之间存在显着的关系，让步比有显著不同。（接受备择假设）</li>
    <li> P值 > = 0.05，则在控制个人吸烟的情况下，二手烟与癌症风险之间没有显着关系。（接受虚假设）</li>
    </ul>"
  ),
conditionalPanel(
    condition = "input.explain_on_off",
     p(tags$i("此默认设置的结论是：在控制个人主动吸烟的情况下，癌症风险与二手烟之间存在显着的关系。 （P <0.001）"))
      )
        )
      )
