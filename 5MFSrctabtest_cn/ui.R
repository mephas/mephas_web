...
<ul>
<li> 测定病例对照状态（行）和因子类别（列）之间是否存在关联
<li> 测定2个独立样本中的比例是否相同
<li> 测定比例是否均质
<li> 获取百分比表和图表以及每个单元格的期望值
</ul>

<h4><b> 2. 计数数据为4单元2×2列联表 </b></h4>
...
headerPanel("病例对照条件下2类小期望计数因子的费雪精确检验"),
...
<h4><b> 1. 功能  </b></h4>
<ul>
<li> 测定病例对照状态（行）和因子状态（列）之间是否存在关联
<li> 测定 2 个相依样本中的比例是否相同
<li> 测定比例是否均质
<li> 获取百分比表和图表以及每个单元格的期望值
...
headerPanel("病例对照条件下2类因子配对计数的McNemar检验"),
...
<ul>
<li> 病例对照结果有2个类别（在行名和列名中）
<li> 因子状态有2个类别（在行名和列名中）
<li> 数据样本为匹配/配对数据
<li> 知道<b>一致对</b>，即匹配对组成的结果均相同
<li> 知道<b>不一致对</b>，即匹配对组成的结果均不同
</ul>

<h4><b> 3. 2×2列联表中的配对计数</b></h4>

<ul>
<li> 两对患者配对，年龄和临床情况相似。一组接受治疗A，另一组接受治疗B，记录有多少人好转，有多少人恶化。
<li> 对于<b>一致对</b>，匹配对组成均改善或恶化
<li> 对于<b>不一致对</b>，匹配对的一个组成改善或恶化
</ul>
...
tabPanel("Kappa统计量(2xK)",

headerPanel("两个评分器可重复性/一致性的Kappa统计量"),
conditionalPanel(
condition = "input.explain_on_off",
HTML(
"
...
<ul>
<li> 两个评分器的结果或两个测量值（例如，Y/N 答案、排名、类别）
</ul>
...
tabPanel("Kappa统计量(KxK)",

headerPanel("重复/相关测量重复性的Kappa统计量"),

conditionalPanel(
condition = "input.explain_on_off",
HTML(
"
<p> 此方法使用不同类型的数据。它使用K×K列联表中所示的一致和不一致计数。</p>

<h4><b> 1. 功能  </b></h4>

<ul>
<li> 对多次测量的同一变量的可重复性进行量化
<li> 量化具有相同结果的2项测量值之间的关联
<li> 获取百分比表及每个单元格的期望值
...
<ul>
<li> 知道<b>结果的一致性</b>、即重复测量中测量结果均相同
<li> 知道<b>结果的不一致</b>、即重复测量中测量结果不同
</ul>
...
headerPanel("K混杂层病例对照条件下2类因子的Mantel-Haenszel检验"),
...
<h4><b> 1. 功能</b></h4>
<ul>
<li> 通过控制分层/混杂测定病例对照状态（行）和因子状态（列）之间是否存在关联
<li> K层中的两个标称变量具有条件独立性
<li> 获取百分比表和图表以及每个单元格的期望值
