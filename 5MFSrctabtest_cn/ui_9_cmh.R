...
conditionalPanel(
   ...
   p(tags$b("备择假设")), 
   p("病例对照（行）与分组因子（列）显著相关；各分层的让步比有显著差异"),     
conditionalPanel(
    condition = "input.explain_on_off",
    ...
    p(tags$b("K层RxC列联表")),
    p("前R行显示第一层的R x C列联表，然后显示第二层的R x C列联表。"),
    ...
    "<b> 説明 </b> 
    <ul> 
    <li> P值 < 0.05，则在控制性别的情况下，习惯性打鼾的患病率与年龄显著相关，让步比也存在显著差异。（接受备择假设）</li>
    <li> P值 > = 0.05，则在控制性别的情况下，习惯性打鼾的患病率与年龄没有显著关系。（接受虚假设）</li>
    </ul>"
  ),
conditionalPanel(
    condition = "input.explain_on_off",
     p(tags$i("此默认设置的结论是：在控制性别的情况下，习惯性打鼾的患病率与年龄显著相关.（P <0.001）"))
        )
      )
    )
