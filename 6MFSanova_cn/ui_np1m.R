...
<li> <b>Bonferroni</b>调整后的p值 = 最大值(1, pm); m= k(k-1)/2 个对的多重比较
<li> <b>Sidak</b>调整后的p值 = 最大值(1, 1 - (1 - p)^m)
<li> <b>Holm</b>法调整后的p值 = 最大值[1, p(m+1-i)]；i是排序索引
<li> <b>Holm-Sidak</b>调整后的p值 = 最大值[1, 1 - (1 - p)^(m+1-i)]
<li> <b>Hochberg</b>法调整后的p值 = 最大值[1, p*i]
<li> <b>Benjamini-Hochberg</b>调整后的p值 = 最大值[1, pm/(m+1-i)]
<li> <b>Benjamini-Yekutieli</b>调整后的p值 = 最大值[1, pmC/(m+1-i)]; C = 1 + 1/2 + ...+ 1/m
...
p(tags$i("如果P值 <= 0.025，则拒绝零假设。"))
),

DT::DTOutput("dunntest.t"),
p(br()),
conditionalPanel(
condition = "input.explain_on_off",
