...
numericInput("n", "实验数/样本数：n，要求n > x", value = 40, min = 1, max = 100000, step = 1),
conditionalPanel(
...
conditionalPanel(
condition = "input.explain_on_off",
p(tags$i("我们要比较的是一般不孕率（20％），p<sub>0</sub>=0.2."))
),
...
plotly::plotlyOutput("makeplot", width = 500),
...
"<b> 说明 </b> 
<ul> 
<li> P 值 < 0.05， 则全体比例/比率和指定比例/比率相比有统计学差异。（接受备择假设）
<li> P 值 >= 0.05， 则全体比例/比率和指定比例/比率相比无统计学差异。（接受零假设）
</ul>"
),
p(br()), 
conditionalPanel(
condition = "input.explain_on_off",
HTML("<i> 根据初始设置，我们可以得到治疗女性的不孕率和一般的20%相比是没有统计学差异的（P值= 0.55）。
在这个示例里，np<sub>0</sub>(1-p<sub>0</sub>)=40*0.2*0.8 > 5。
因此，适用 <b>正态理论方法（Normal Theory Method）</b> 。 </i>")
)
