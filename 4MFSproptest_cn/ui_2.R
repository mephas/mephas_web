...
numericInput("n1", "实验数/样本数，n1，要求n1 > x1", value = 3220, min = 1, max = 10000000, step = 1),
conditionalPanel(
...
numericInput("n2", "实验数/样本数（总数），n2，要求n2 > x2", value = 10245, min = 1, max = 10000000, step = 1),
conditionalPanel(
condition = "input.explain_on_off",
...
plotly::plotlyOutput("makeplot2", width = 500),
p(br()), 

p(tags$b("2. 控制组")),
p(br()), 
plotly::plotlyOutput("makeplot2.1", width = 500),
...
"<b> 说明 </b> 
<ul> 
<li> P值 < 0.05， 则两组的总比例/比率有统计学显著差异。（接受备择假设）
<li> P值 >= 0.05， 则两组的总比例/比率无统计学显著差异。（接受零假设）
</ul>"
),
p(br()), 
conditionalPanel(
